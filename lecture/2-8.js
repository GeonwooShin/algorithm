...
function solution(arr, target) {
  return plusOrMinus(arr, target, 0, 0);
}

function plusOrMinus(arr, target, currentIdx, currentSum) {
  if (currentIdx === arr.length) {
    return currentSum === target ? 1 : 0;
  }

  const addCount = plusOrMinus(
    arr,
    target,
    currentIdx + 1,
    currentSum + arr[currentIdx]
  );
  const minusCount = plusOrMinus(
    arr,
    target,
    currentIdx + 1,
    currentSum - arr[currentIdx]
...
const answer = solution(numbers, target_number);
console.log(answer);
